...
const SliceAdder = props => {
  ...
  const [selectedSliceIdsSet, setSelectedSliceIdsSet] = useState(
    new Set(props.selectedSliceIds),
  );
  const [showOnlyMyCharts, setShowOnlyMyCharts] = useState(
    getItem(LocalStorageKeys.dashboard__editor_show_only_my_charts, true),
  );
  ...
  const getFilteredSortedSlicesHandler = useCallback(
  ...
  const searchUpdatedHandler = useCallback(
    searchTerm => {
      setStateHandler(prevState => ({
        searchTerm,
        filteredSlices: getFilteredSortedSlicesHandler(
          props.slices,
          searchTerm,
          prevState.sortBy,
  ...
  const rowRendererHandler = useCallback(
    ({ key, index, style }) => {
  ...
  const onShowOnlyMyChartsHandler = useCallback(
    showOnlyMyCharts => {
      if (!showOnlyMyCharts) {
        slicesRequestHandler = props.fetchSlices(undefined, searchTerm, sortBy);
      }
      setStateHandler(prevState => ({
        showOnlyMyCharts,
        filteredSlices: getFilteredSortedSlicesHandler(
