...
const TTestTable = props => {
  ...
  const getLiftStatusHandler = useCallback(
    row => {
  ...
  const getPValueStatusHandler = useCallback(
    row => {
      if (row === control) {
        return 'control';
      }
  ...
  const getSignificanceHandler = useCallback(
    row => {
      const { alpha } = props;
  ...
  const computeLiftHandler = useCallback(
    (values, control) => {
  ...
  const computePValueHandler = useCallback(
    (values, control) => {
  ...
  const computeTTestHandler = useCallback(
    control => {
      ...
      for (let i = 0; i < data.length; i += 1) {
        if (i === control) {
          ...
          pValues.push(
            computePValueHandler(data[i].values, data[control].values),
          );
          liftValues.push(
            computeLiftHandler(data[i].values, data[control].values),
          );
        }
      }
      setControl(control);
