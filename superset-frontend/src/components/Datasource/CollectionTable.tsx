...
const CRUDCollection = (props: CRUDCollectionProps) => {
  ...
  const onCellChangeHandler = useCallback(
    (id: number, col: string, val: boolean) => {
  ...
  const onFieldsetChangeHandler = useCallback(
    (item: any) => {
      changeCollectionHandler({
        ...collection,
        [item.id]: item,
  ...
  const changeCollectionHandler = useCallback(
    (collection: any, newItem?: object) => {
      setCollection(collection);
      if (props.onChange) {
  ...
  const sortColumnHandler = useCallback(
    (col: string, sort = SortOrder.unsort) => {
      ...
      return () => {
        if (sortColumns?.includes(col)) {
          ...
          const sorted = [...collectionArray].sort((a: object, b: object) =>
  ...
  const renderSortIconHandler = useCallback(
    (col: string) => {
  ...
  const renderTHHandler = useCallback(
    (col: string, sortColumns: Array<string>) => {
  ...
  const renderHeaderRowHandler = useCallback(() => {
    ...
    const { allowDeletes, expandFieldset, extraButtons, sortColumns } = props;
    return (
      <thead>
        <tr>
  ...
  const renderExpandableSectionHandler = useCallback((item: any) => {
    ...
    return recurseReactClone(props.expandFieldset, Fieldset, propsGenerator);
  }, []);
  ...
  const renderCellHandler = useCallback((record: any, col: any) => {
    ...
    return renderer
      ? renderer(val, onChange, getLabelHandler(col), record)
      : val;
  }, []);
  const renderItemHandler = useCallback(
    (record: any) => {
      ...
      const isExpanded = !!expandedColumns[record.id] || record.__expanded;
      let tds = [];
      if (expandFieldset) {
        tds.push(
  ...
  return (
    <>
      ...
      <CrudTableWrapper className="CRUD" stickyHeader={props.stickyHeader}>
